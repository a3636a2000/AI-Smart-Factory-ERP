...
import { Search, Plus, Save, Trash2, Printer, FileSpreadsheet, FileUp, X, Home, Globe, Bell, Download, Settings, LayoutDashboard, FileText, Activity, Factory, Eye, AlertTriangle, Package, Image, Video, MessageSquare, HelpCircle, Shield, FileCheck, Cookie } from 'lucide-react'
...
interface LayoutProps {
  ...
  const TAB_ICONS: Record<string, { icon: React.ElementType; className: string }> = {
    '/sf-dashboard': { icon: LayoutDashboard, className: 'text-blue-600' },
    '/sf-production': { icon: Factory, className: 'text-slate-500' },
    '/sf-vision': { icon: Eye, className: 'text-cyan-600' },
    '/sf-vision-setup': { icon: Settings, className: 'text-blue-500' },
    '/sf-defect-types': { icon: AlertTriangle, className: 'text-amber-500' },
    '/sf-items': { icon: Package, className: 'text-slate-500' },
    '/ai-data-setup': { icon: Settings, className: 'text-slate-500' },
    '/ai-ontology': { icon: LayoutDashboard, className: 'text-blue-600' },
    '/ai-pattern': { icon: FileText, className: 'text-emerald-600' },
    '/ai-insight': { icon: Activity, className: 'text-indigo-500' },
    '/pdf-converter': { icon: FileText, className: 'text-slate-500' },
    '/image-editor': { icon: Image, className: 'text-slate-500' },
    '/video-maker': { icon: Video, className: 'text-slate-500' },
    '/chat': { icon: MessageSquare, className: 'text-slate-500' },
    '/help-center': { icon: HelpCircle, className: 'text-blue-500' },
    '/privacy-policy': { icon: Shield, className: 'text-indigo-500' },
    '/terms-of-service': { icon: FileCheck, className: 'text-emerald-600' },
    '/cookie-policy': { icon: Cookie, className: 'text-amber-500' },
  }
      ...
      <header className="h-14 bg-white text-slate-800 border-b border-slate-200 flex items-center shrink-0 z-20">
        ...
        <div className="flex items-center gap-4 shrink-0 px-4 ml-auto">
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
            <img src="https://ui-avatars.com/api/?name=HM&background=eff6ff&color=2563eb" alt="Profile" className="w-7 h-7 rounded-full" />
          ...
          <div className="h-10 mt-1 flex items-center px-2 bg-transparent overflow-x-auto shrink-0 z-10 w-full relative">
            ...
            <div className="flex items-end flex-1 overflow-x-auto gap-1 px-1 h-full pt-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {tabs.map((tab) => {
                ...
                const tabIcon = TAB_ICONS[tab.id] ?? { icon: FileText, className: 'text-slate-400' }
                ...
                return (
                  ...
                  >
                    <TabIcon className={cn('w-3.5 h-3.5 shrink-0', isActive ? tabIcon.className : 'text-slate-400 group-hover:text-slate-500')} />
                    {tab.label}
                    {tabs.length > 1 && (
                      <button 

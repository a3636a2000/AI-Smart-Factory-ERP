...
const DEFAULT_CHANNELS = [
  { id: 'general', name: '일반', description: '자유롭게 대화하세요', type: 'chat' as const, icon: MessageCircle },
  { id: 'questions', name: '질문', description: 'AI 한글 에디터 사용법 질문', type: 'chat' as const, icon: HelpCircle },
  { id: 'tips', name: '팁 공유', description: '유용한 팁과 노하우를 공유', type: 'chat' as const, icon: Lightbulb },
  { id: 'feedback', name: '피드백', description: '기능 제안 및 개선 요청', type: 'chat' as const, icon: MessageSquarePlus },
  { id: 'feed', name: '피드 게시판', description: '자유 게시판 - 글, 이미지, 동영상 공유', type: 'feed' as const, icon: Newspaper },
]
...
export default function ChatRoom() {
  ...
  return (
    <SmartFactoryWrapper>
      <div className="flex flex-col absolute inset-0 p-2 overflow-hidden bg-[#f8f9fc] animate-in slide-in-from-bottom-4 fade-in duration-500 min-h-0">
      {/* 페이지 헤더 */}
      <div className="relative bg-white border border-indigo-100 rounded-xl px-4 py-3 mb-2 flex items-center gap-3 shrink-0 shadow-sm">
        <MessageCircle className="w-5 h-5 text-indigo-600" />
        <div>
          <h2 className="text-indigo-900 font-bold text-lg tracking-tight">채팅</h2>
          <p className="text-slate-500 text-[11px] mt-0.5">채널 대화 · 피드 게시판 · 팀 공유</p>
        </div>
      </div>

      <div className="flex flex-1 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm min-h-0">
      {/* ─── 모바일 오버레이 배경 ─── */}
      {showChannelSidebar && (
        <div
      ...
      <div
        className={`
          ...
          w-60 border-r border-slate-200 flex flex-col shrink-0
      ...
      >
        ...
        <div className="h-12 px-4 flex items-center border-b border-slate-200 bg-white">
          <h2 className="font-semibold text-indigo-900 text-sm truncate">AI 한글 에디터</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 px-2">
          ...
          {DEFAULT_CHANNELS.filter(ch => ch.type === 'chat').map((ch) => {
            ...
            >
              <ChannelIcon className={`w-4 h-4 shrink-0 ${activeChannel === ch.id ? 'text-indigo-600' : 'opacity-60'}`} />
        ...
        {/* 유저 패널 (하단) */}
        <div className="h-[52px] px-2 flex items-center bg-white border-t border-slate-200">
          {nickname ? (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-slate-500">접속 중</p>
              </div>
              <button
                onClick={() => {
            ...
            <button
              ...
              className="flex items-center gap-2 text-sm text-slate-500 hover:text-indigo-600 transition-colors w-full"
            >

  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh] rounded-xl border border-slate-200 bg-white backdrop-blur-xl shadow-2xl" onKeyDown={handleKeyDown}>
        <div className="bg-white border-b border-indigo-100 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="text-indigo-600 shrink-0">{icon || <Package className="w-5 h-5" />}</span>
            <div>
              <h3 className="text-indigo-900 font-bold text-base tracking-tight">{title}</h3>
              <p className="text-slate-500 text-[11px] mt-0.5">검색 후 항목을 선택하세요</p>
            </div>
          </div>
        ...
        <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-slate-800 placeholder-slate-400"
            />
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((item, idx) => (
                <tr
                  key={idx}
                  className={`border-b border-slate-200 cursor-pointer transition-colors ${focusedIdx === idx ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}
                ...
                >
                  {columns.map(col => (
                    <td key={col.key} className="px-4 py-2.5">
                      {col.isCode
                        ? <span className="text-indigo-600 font-semibold text-xs">{item[col.key]}</span>
...
function BarcodeScanModal({ isOpen, onClose, onScan, title }: BarcodeScanModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-xl border border-indigo-100 bg-white shadow-xl">
        {/* 헤더 */}
        <div className="bg-white border-b border-indigo-100 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <QrCode className="w-5 h-5 text-indigo-600 shrink-0" />
            <div>
              <h3 className="text-indigo-900 font-bold text-base tracking-tight leading-tight">{title}</h3>
              <p className="text-slate-500 text-[11px] mt-0.5">카메라 또는 직접 입력으로 스캔</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* 카메라 영역 */}
        <div className="bg-[#f1f5f9] relative aspect-video flex items-center justify-center border-b border-slate-200">
          ...
          {!scanning && (
            <div className="text-slate-400 flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-slate-500">카메라를 시작하여 바코드/QR을 스캔하세요</p>
            </div>
          )}
          {scanning && (
            <>
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="w-48 h-32 border-2 border-indigo-400 rounded-lg relative shadow-[0_0_0_9999px_rgba(15,23,42,0.35)]">
                  <span className="absolute -top-1 -left-1 w-4 h-4 border-t-2 border-l-2 border-indigo-500" />
                  <span className="absolute -top-1 -right-1 w-4 h-4 border-t-2 border-r-2 border-indigo-500" />
                  <span className="absolute -bottom-1 -left-1 w-4 h-4 border-b-2 border-l-2 border-indigo-500" />
                  <span className="absolute -bottom-1 -right-1 w-4 h-4 border-b-2 border-r-2 border-indigo-500" />
                  <div className="absolute inset-x-0 top-0 h-0.5 bg-indigo-400 animate-scan" />
                </div>
              </div>
              <div className="absolute top-3 right-3 bg-white/90 border border-indigo-100 text-indigo-700 text-[11px] font-semibold px-2.5 py-1 rounded-md flex items-center gap-1.5 shadow-sm">
                <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> SCANNING
              </div>
            </>
          )}
        </div>
        ...
        {lastScanned && (
          <div className="px-4 py-2.5 bg-emerald-50 border-b border-emerald-100 flex items-center gap-2">
            <span className="text-emerald-700 text-xs font-semibold">스캔됨:</span>
            <span className="font-mono text-sm text-emerald-800">{lastScanned}</span>
          </div>
        )}
        ...
        <div className="p-4 space-y-3 bg-white">
          <div className="flex gap-2">
            {!scanning
              ? (
                <Button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm" onClick={startScan}>
                  <Camera className="w-4 h-4 mr-2" /> 카메라 스캔 시작
                </Button>
              )
              : (
                <Button className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200" onClick={stopScan}>
                  <X className="w-4 h-4 mr-2" /> 스캔 중지
                </Button>
              )}
          </div>
          <div className="relative flex items-center">
            <div className="flex-1 border-t border-slate-200" />
            <span className="px-3 text-xs text-slate-400">또는 직접 입력</span>
            <div className="flex-1 border-t border-slate-200" />
          </div>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 h-9 px-3 text-sm bg-slate-50 border border-slate-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-300"
            />
            <Button
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button className="h-9 w-9 rounded-md bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200 flex items-center justify-center transition-colors" onClick={() => setWorkOrderScanOpen(true)}><QrCode className="w-4 h-4" /></button>

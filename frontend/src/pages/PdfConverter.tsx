...
import { type ModelTier, MODEL_TIERS, getModelTier, setModelTier as saveModelTier, getTextModel } from '@ai/gemini/modelConfig'
...
const PdfConverter = () => {
  ...
  return (
    <SmartFactoryWrapper>
      <div className="flex flex-col absolute inset-0 bg-[#f8f9fc] p-2 overflow-auto animate-in slide-in-from-bottom-4 fade-in duration-500">
      ...
      <>
      {/* 페이지 헤더 */}
      <div className="relative bg-white border border-indigo-100 rounded-xl px-4 py-3 mb-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          <FileDown className="w-5 h-5 text-indigo-600" />
          <div>
            <h2 className="text-indigo-900 font-bold text-lg tracking-tight">PDF / 이미지 변환</h2>
      ...
      {/* 워터마크 제거 토글 */}
      <div className="flex items-center gap-3 mb-2 flex-wrap bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
        <button
          onClick={() => setWatermarkRemoval(!watermarkRemoval)}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ${
            watermarkRemoval ? 'bg-indigo-600' : 'bg-slate-200'
          }`}
        >
          <span
        ...
        <div className="flex items-center gap-2">
          <Eraser className={`w-4 h-4 ${watermarkRemoval ? 'text-indigo-500' : 'text-slate-400'}`} />
          <span className={`text-sm font-medium ${watermarkRemoval ? 'text-indigo-900' : 'text-slate-500'}`}>
            워터마크 자동 제거
          </span>
        </div>
        {watermarkRemoval && (
          <span className="text-xs text-slate-500 bg-indigo-50 px-2 py-0.5 rounded-md hidden sm:inline border border-indigo-100">
      ...
      {!file && (
        <div
          className="border-2 border-dashed border-indigo-200 bg-white rounded-xl sm:rounded-2xl p-8 sm:p-12 text-center hover:border-indigo-400 hover:bg-indigo-50/40 active:border-indigo-500 transition-colors cursor-pointer mb-4 sm:mb-6 shadow-sm"
          onDrop={handleDrop}
        ...
        >
          ...
          />
          <Upload className="w-10 h-10 sm:w-12 sm:h-12 text-indigo-400 mx-auto mb-3 sm:mb-4" />
          <p className="text-indigo-900 font-semibold text-base sm:text-lg mb-2">
            PDF 또는 이미지 파일을 여기에 드래그하세요
          </p>
          <p className="text-slate-500 text-xs sm:text-sm">또는 클릭하여 파일 선택 · PDF, JPG, PNG 등 지원 (최대 50MB)</p>
        </div>
      )}

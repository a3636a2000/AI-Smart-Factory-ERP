...
const ImageEditor = () => {
  ...
  return (
    <SmartFactoryWrapper>
      <div className="flex flex-col absolute inset-0 p-2 min-h-0 animate-in slide-in-from-bottom-4 fade-in duration-500 bg-[#f8f9fc]">
      {/* 페이지 헤더 */}
      <div className="relative bg-white border border-indigo-100 rounded-xl px-4 py-3 mb-2 flex items-center gap-3 shrink-0 shadow-sm">
        ...
        <div>
          <h2 className="text-indigo-900 font-bold text-lg tracking-tight">이미지 편집</h2>
          <p className="text-slate-500 text-[11px] mt-0.5">깨진 한글 텍스트를 인식·교체하고 영역을 편집합니다</p>
        </div>
      </div>

      {/* ═══ 상단 툴바 ═══ */}
      <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 mb-2 bg-white border border-slate-200 rounded-xl shadow-sm shrink-0 flex-wrap">
        {/* 이미지 업로드 */}
        <button
          onClick={() => fileInputRef.current?.click()}
          className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs sm:text-sm transition-colors font-medium shadow-sm"
        >
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden relative">
        ...
        >
          {!editCanvas ? (
            <div
              className="border-2 border-dashed border-indigo-200 bg-white rounded-xl sm:rounded-2xl p-8 sm:p-16 text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/40 active:border-indigo-500 transition-colors shadow-sm"
              onClick={() => fileInputRef.current?.click()}
            >
              <Upload className="w-12 h-12 sm:w-16 sm:h-16 text-indigo-400 mx-auto mb-3 sm:mb-4" />
              <p className="text-indigo-900 font-semibold text-base sm:text-lg mb-2">이미지를 업로드하세요</p>

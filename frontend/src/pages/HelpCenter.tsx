    ...
    <SmartFactoryWrapper>
      ...
      <div className="relative bg-white border border-indigo-100 rounded-xl px-4 py-3 mb-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-indigo-900 font-bold text-lg tracking-tight">고객센터</h2>
            <p className="text-slate-500 text-[11px] mt-0.5">궁금한 것이 있으시면 언제든지 문의해 주세요</p>
          </div>
        </div>
        <div className="relative w-full sm:w-72 shrink-0">
          <input
            ...
            className="w-full pl-4 pr-12 py-2 rounded-lg bg-slate-50 text-slate-800 text-sm placeholder-slate-400 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-300"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          <button className="absolute right-1 top-1 bottom-1 px-3 bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors">
      ...
      <div className="max-w-5xl mx-auto w-full">
        <div className="flex border-b border-slate-200 mb-6 overflow-x-auto bg-white rounded-t-xl px-2 border border-b-0 border-slate-200">
          {[
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                <h3 className="font-semibold mb-3 text-indigo-900 text-sm">카테고리</h3>
                <div className="space-y-1">
                  {categories.map((cat) => (
                    <button
                      key={cat.id}
                      onClick={() => setSelectedCategory(cat.id)}
                      className={`w-full text-left px-3 py-2 rounded-lg transition-colors flex items-center text-sm gap-2 ${
            ...
            {notices.map((n) => (
              <AccordionItem
                ...
                badge={
                  <>
                    {n.important && (
                      <span className="text-xs bg-indigo-50 border border-indigo-200 text-indigo-700 rounded px-2 py-0.5 shrink-0 font-medium">
                        중요
                      </span>
                    )}

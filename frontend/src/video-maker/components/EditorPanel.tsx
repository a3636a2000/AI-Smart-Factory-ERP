...
export const EditorPanel: React.FC<EditorPanelProps> = ({
  ...
  if (!slide) {
    return (
      <div className="h-72 bg-[#f8f9fc] border-t border-slate-200 p-8 flex items-center justify-center text-slate-400">
        <p className="text-sm font-medium">슬라이드를 선택하세요</p>
      </div>
    )
  }

  return (
    <div className="h-80 bg-white flex flex-col md:flex-row">
      {/* ── 나레이션 대본 섹션 ── */}
      <div className="flex-1 p-4 sm:p-5 border-b md:border-b-0 md:border-r border-slate-200 flex flex-col gap-3">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <label className="text-xs font-semibold text-indigo-900">나레이션 대본 (TTS)</label>

          <div className="flex items-center gap-2">
            <select
              className="bg-white border border-slate-200 text-xs rounded-md px-2 py-1 focus:ring-indigo-500 text-slate-700 outline-none"
              value={scriptLevel}
        ...
        <textarea
          className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm text-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-300 resize-none leading-relaxed outline-none"
        ...
        <div className="flex items-center gap-3">
          <select
            className="bg-white border border-slate-200 text-sm rounded-md px-3 py-1.5 focus:ring-indigo-500 outline-none text-slate-700"
            value={selectedVoice}
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-all
              ${slide.isGeneratingAudio ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm'}`}
          >
      ...
      {/* ── 자막 스타일 설정 섹션 ── */}
      <div className="w-full md:w-1/3 p-4 sm:p-5 flex flex-col gap-3 bg-slate-50 border-l border-slate-200 overflow-y-auto">
        <label className="text-xs font-semibold text-indigo-900">자막 스타일</label>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full accent-indigo-500 bg-slate-200 h-1.5 rounded-full appearance-none cursor-pointer"
            />
          </div>
          ...
          {/* 글꼴 종류 */}
          <div className="space-y-1">
            <label className="text-xs text-slate-500">글꼴</label>
            <select
              value={subtitleStyle.fontFamily}
              onChange={(e) => updateStyle('fontFamily', e.target.value)}
              className="w-full bg-white border border-slate-200 rounded-md px-2 py-1.5 text-sm text-slate-700 focus:ring-indigo-500 outline-none"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs text-slate-500">글자 색상</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={subtitleStyle.color}
                  onChange={(e) => updateStyle('color', e.target.value)}
                  className="w-8 h-8 rounded border border-slate-200 bg-white cursor-pointer"
                />
                <span className="text-xs text-slate-500 uppercase">{subtitleStyle.color}</span>
              </div>
            </div>

            <div className="space-y-1">
              <label className="text-xs text-slate-500">배경 색상</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  ...
                  className="w-8 h-8 rounded border border-slate-200 bg-white cursor-pointer"
                />
                <span className="text-xs text-slate-500 uppercase">{subtitleStyle.backgroundColor}</span>
              </div>
            </div>
          </div>

  ...
  return (
    <div
      className={`group relative flex flex-col gap-2 p-2 rounded-lg cursor-pointer transition-all border-2 
        ${isActive ? 'border-indigo-500 bg-indigo-50' : 'border-transparent hover:bg-slate-50'}
        ${isSelected && !isActive ? 'bg-indigo-50/50 border-indigo-200' : ''}`}
      onClick={onClick}
    >
      <div className="relative aspect-video w-full bg-slate-100 rounded overflow-hidden shadow-sm border border-slate-200">
